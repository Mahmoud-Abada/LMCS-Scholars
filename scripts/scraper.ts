...
export class ResearchDataScraper {
  ...
  private async scrapeJournalSJR(journalName: string): Promise<[string, string] | null> {
  const page = await this.browser!.newPage();
  try {
    ...
    await page.waitForSelector('.search_results > a, .nores', { timeout: 10000 });
    ...
    const sjrBlock = await page.locator('h2:has-text("SJR 2024") + p.hindexnumber').first();
