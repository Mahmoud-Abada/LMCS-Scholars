  ...
  try {
    ...
    if (accLink) {
      ...
      while (true) {
        try {
          ...
          if (!loadMore) break;
          const isDisabled = await page.$eval("#gsc_bpf_more", (el) =>
    ...
    if (dblpPublishes.length > 0) {
      ...
      for (let i = 0; i < dblpPublishes.length; i++) {
        ...
        if (hasGoogleScholar && allGoogleScholarTitles) {
          matchIndex = allGoogleScholarTitles
            .toArray()
            .findIndex((el) => isSimilar($$(el).text(), title));
          if (matchIndex !== -1) {
            const scholarLink = $$(allGoogleScholarTitles[matchIndex]).attr(
              "href"
            );
            pubUrl = scholarLink
              ? `https://scholar.google.com${scholarLink}`
              : "";
        ...
        if (jOrC?.[1] === "c") {
          ...
          if (confLink) {
            try {
              ...
              console.warn(
                `Failed to fetch conference location from ${confLink}: ${error}`
              );
            }
          }
        }
    ...
    if (!hasGoogleScholar && !hasDBLP) {
      console.warn(
        `❌ No Google Scholar or DBLP profile found for "${chercheurName}"`
      );
      await browser.close();
      return [];
    }
